...
/*
内部定时器中断 TIM  --->  NVIC  --->  CPU
频率：单位时间内完成周期性变化的次数（下面讲的频率都是以1s为单位时间）
时钟周期=频率的倒数

因为以1s为单位时间，则 Hz 和 s 是相对应的单位
...
72MHz的时钟周期为1s/72MHz = 1us/72Hz = 1/72 us      us 对应 M   ms 对应 K

对于计数器来说，每来一个脉冲记一个数，所以如果计72个数，则需要72个脉冲，耗费时间为 1/72 * 72 = 1us

所以如果计72000个数则耗费时间为1/72 * 72000 = 1ms



16位的计数器CNT、预分频器PSC(16位)、自动重装寄存器ARR(16位) 三个一起称为 时基单元

预分频器：对计数器的时钟进行分频，让计数更加灵活 0一分频 1二分频，将分频后的频率传给CNT计数器
自动重装寄存器是：计数的目标值，就是我想要计多少个数，记满后清0，此时可以触发一些事件或中断
...
计数器溢出频率（计数记满的频率）：CK_CNT_OV = CK_CNT / (ARR + 1) = CK_PSC / (PSC + 1) / (ARR + 1)
溢出频率＝计数频率/(重装目标值+1) 
*/
void Timer_Init(void)
{
	...
	/******* 2.选择内部时钟 *******/
	TIM_InternalClockConfig(TIM2); // 将TIM2作为内部时钟使用
	/******* 3.配置时基单元，开启时钟中断通道 *******/
